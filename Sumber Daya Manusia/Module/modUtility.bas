...
Function Periksa(vCase As String, vObj As Object, vPesan As String) As Boolean
    ...
    Select Case LCase(vCase)
        ...
        Case "datepicker"
            If Val(vObj) > 0 Then Exit Function
        Case "MaskEdBox"
            If vObj.Text <> "__:__" Then Exit Function
    End Select
...
Public Sub subSetSubTotalRow(vobjForm As Form, mintRowNow As Double, iColBegin As Integer, vbBackColor, vbForeColor)
    Dim i As Integer
...
Public Sub openConnection()
    ...
    With dbConn
        ...
        'sa
        '.ConnectionString = "Provider=SQLNCLI10.1;Server=" & strServerName & ";Database=" & strDatabaseName & ";Uid=" & strUserName & "; Pwd=" & strPassword & ";"
...
Public Sub msubPesanError(Optional s_Sub As String)
	...
	If (Err.Description = "Communication link failure") Then
        dbConn.Close
        Call openConnection
    End If
End Sub
...
Public Function funcRound(strNumber As String, dblPer As Double) As Double
    ...
    If (CDbl(Right(strNum, intLenNum)) Mod dblPer) < dblPer And CDbl(Right(strNum, intLenNum)) <> 0 Then
        ...
        'funcRound = dblNumLeft + (funcRoundUp(CDbl(strNum) / dblPer) * dblPer)
...
Function NumToTextRupiah(dblValue As Double) As String
    Static ones(0 To 9) As String
    Static teens(0 To 9) As String
    Static tens(0 To 9) As String
    Static thousands(0 To 4) As String
    ...
    Select Case mstrChoose
        Case "TglLahir"
            ...
            If Mid(mobjObject.Text, 4, 2) > 12 Then
                MsgBox "Bulan tanggal lahir tidak boleh melebihi bulan 12!", vbCritical, "Informasi"
            ...
            If CInt(Mid(mobjObject.Text, 1, 2)) > funcHitungHari(CInt(Mid(mobjObject.Text, 4, 2)), CInt(Mid(mobjObject.Text, 7, 4))) Then
                MsgBox "Hari tanggal lahir yang dimasukkan lebih besar dari jumlah hari dari bulan tanggal lahir yang dimasukkan!", vbCritical, "Informasi"
            ...
            If Year(mobjObject.Text) > Year(Now) Then
                MsgBox "Tahun tanggal lahir tidak boleh melebihi tahun sekarang!", vbCritical, "Informasi"
...
lanjut:
            If Format(mobjObject.Text, "yyyy/MM/dd") > Format(Now, "yyyy/MM/dd") Then
                MsgBox "Tanggal lahir tidak boleh melebihi tanggal sekarang!", vbCritical, "Informasi"
                mobjObject.SelStart = 6
                mobjObject.SelLength = 4
                mobjObject.SetFocus
                funcCekValidasiTgl = "ErrTahun"
...
errTglLahir:
        ...
        Case "TglMasuk"
            ...
            End If
            If Mid(mobjObject.Text, 4, 2) > 12 Then
                MsgBox "Bulan tanggal masuk tidak boleh melebihi bulan 12!", vbCritical, "Informasi"
                mobjObject.SelStart = 3
                mobjObject.SelLength = 2
                mobjObject.SetFocus
                funcCekValidasiTgl = "ErrTahun"
            ...
            End If
            If CInt(Mid(mobjObject.Text, 1, 2)) > funcHitungHari(CInt(Mid(mobjObject.Text, 4, 2)), CInt(Mid(mobjObject.Text, 7, 4))) Then
                MsgBox "Hari tanggal masuk yang dimasukkan lebih besar dari jumlah hari dari bulan tanggal masuk yang dimasukkan!", vbCritical, "Informasi"
                mobjObject.SelStart = 0
                mobjObject.SelLength = 2
                mobjObject.SetFocus
                funcCekValidasiTgl = "ErrTahun"
            ...
            End If
            If Year(mobjObject.Text) > Year(Now) Then
                MsgBox "Tahun tanggal masuk tidak boleh melebihi tahun sekarang!", vbCritical, "Informasi"
                mobjObject.SelStart = 6
                mobjObject.SelLength = 4
                mobjObject.SetFocus
                funcCekValidasiTgl = "ErrTahun"
...
teruskan:
            If Format(mobjObject.Text, "yyyy/MM/dd") > Format(Now, "yyyy/MM/dd") Then
                MsgBox "Tanggal masuk tidak boleh melebihi tanggal sekarang!", vbCritical, "Informasi"
                mobjObject.SelStart = 6
                mobjObject.SelLength = 4
                mobjObject.SetFocus
                funcCekValidasiTgl = "ErrTahun"
...
errgoTglLahir:
        ...
        Case "Jam"
            ...
            If mobjObject.Text = "__:__" Then
                funcCekValidasiTgl = "ErrEmpty"
                Exit Function
            End If
            If Mid(mobjObject.Text, 4, 2) > 59 Then
                MsgBox "Menit tidak boleh melebihi menit 59!", vbExclamation, "Validasi"
                mobjObject.SelStart = 3
                mobjObject.SelLength = 2
                mobjObject.SetFocus
                funcCekValidasiTgl = "ErrMenit"
                Exit Function
            End If
            If Mid(mobjObject.Text, 1, 2) > 23 Then
                MsgBox "Jam tidak boleh melebihi Jam 23!", vbExclamation, "Validasi"
                mobjObject.SelStart = 0
                mobjObject.SelLength = 2
                mobjObject.SetFocus
                funcCekValidasiTgl = "ErrJam"
                Exit Function
            End If
            ...
            Exit Function
ErrJam:
            MsgBox "Format Data jam Salah, Format yang Benar Adalah : HH:mm", vbCritical, "Informasi"
            mobjObject.SelStart = 0
            mobjObject.SelLength = Len(mobjObject.Text)
            mobjObject.SetFocus
            funcCekValidasiTgl = "ErrFormat"
...
Public Function funcPrepareString(strValue As String) As String
    strValue = Trim$(strValue)
    If Len(strValue) = 0 Then
        funcPrepareString = "Null"
    Else
        funcPrepareString = "'" & strValue & "'"
    End If
End Function

Public Function msubKonversiTitikKoma(f_Nilai As String)
    Dim tempKomaTitik  As String
    Dim j As Integer
    ...
    For j = 1 To Len(tempKomaTitik)
        ...
        End If
    Next j
    msubKonversiTitikKoma = tempKomaTitik
End Function
