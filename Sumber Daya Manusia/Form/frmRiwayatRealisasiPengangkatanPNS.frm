...
Begin VB.Form frmRiwayatRealisasiPengangkatanPNS 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Medifirst2000 - Riwayat Realisasi Pengangkatan Pegawai Negeri Sipil"
   ClientHeight    =   8475
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   9735
   Icon            =   "frmRiwayatRealisasiPengangkatanPNS.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ScaleHeight     =   8475
   ScaleWidth      =   9735
   Begin VB.TextBox txtNoRiwayatUsulan 
      Appearance      =   0  'Flat
      BackColor       =   &H8000000E&
      BeginProperty Font 
         Name            =   "Tahoma"
   ...
   Begin VB.Frame fraPerjalanan 
      Caption         =   "Realisasi Pengangkatan"
      BeginProperty Font 
         Name            =   "Tahoma"
         Size            =   8.25
         Charset         =   0
      ...
      Begin MSComCtl2.DTPicker dtpTglSK 
         ...
         CurrentDate     =   38448
      End
      Begin MSDataListLib.DataCombo dcDetailKategory 
         Height          =   315
         Left            =   240
         TabIndex        =   0
         Top             =   600
      ...
      Begin VB.Label Label7 
         AutoSize        =   -1  'True
         Caption         =   "Detail Kategory Disetujui"
         BeginProperty Font 
            Name            =   "Tahoma"
            Size            =   8.25
            Charset         =   0
   ...
   Begin VB.Image Image2 
      Height          =   945
      Left            =   7920
      Picture         =   "frmRiwayatRealisasiPengangkatanPNS.frx":0CCA
      Stretch         =   -1  'True
      Top             =   0
      Width           =   1875
   End
   Begin VB.Image Image3 
      Height          =   975
      Left            =   0
      Picture         =   "frmRiwayatRealisasiPengangkatanPNS.frx":1A52
      Stretch         =   -1  'True
      Top             =   0
      Width           =   1800
   End
   Begin VB.Image Image1 
      Height          =   975
      Left            =   1800
      Picture         =   "frmRiwayatRealisasiPengangkatanPNS.frx":4413
      Stretch         =   -1  'True
      Top             =   0
      Width           =   9495
   End
End
Attribute VB_Name = "frmRiwayatRealisasiPengangkatanPNS"
...
Private Sub cmdSimpan_Click()
    ...
    If Periksa("datacombo", dcDetailKategory, "detail kategori pegawai realisasi kosong ") = False Then Exit Sub
...
Private Sub dcDetailKategory_KeyPress(KeyAscii As Integer)
    If KeyAscii = 13 Then txtGaji.SetFocus
End Sub
...
Private Sub dgData_RowColChange(LastRow As Variant, ByVal LastCol As Integer)
    ...
    With dgData
        ...
        dcDetailKategory.BoundText = .Columns(5).Value
        txtGaji.Text = .Columns(7).Value
        If IsNull(.Columns(10).Value) Then dtpSetuju.Value = Null Else dtpSetuju.Value = .Columns(10).Value
        If IsNull(.Columns(11).Value) Then dtpBerlaku.Value = Null Else dtpBerlaku.Value = .Columns(11).Value
...
Private Sub Form_Load()
    ...
    Call subClearData
    Call SetComboDetailKategori
    Call subLoadRiwayatRealisasi
End Sub

Private Sub subLoadRiwayatRealisasi()
    On Error GoTo errLoad
    strSQL = "SELECT dbo.RiwayatRealisasiUsulan.NoRiwayat, dbo.RiwayatRealisasiUsulan.IdPegawai, dbo.RiwayatRealisasiUsulan.TglSK, " & _
    "dbo.RiwayatRealisasiUsulan.NoSK, dbo.RiwayatRealisasiUsulan.TandaTanganSK, dbo.RiwayatRealisasiUsulan.KdDKategoryPDiSetujui, " & _
    "dbo.DetailKategoryPegawai_M.DetailKategoryPegawai, dbo.RiwayatRealisasiUsulan.GajiPokokDiSetujui, " & _
    "dbo.RiwayatRealisasiUsulan.KeteranganLainnya, dbo.RiwayatRealisasiUsulan.NoRiwayatUsulan, dbo.RiwayatRealisasiUsulan.TglDiSetujui, " & _
    "dbo.RiwayatRealisasiUsulan.TglBerlaku " & _
    "FROM dbo.RiwayatRealisasiUsulan INNER JOIN " & _
    "dbo.DetailKategoryPegawai_M ON dbo.RiwayatRealisasiUsulan.KdDKategoryPDiSetujui = dbo.DetailKategoryPegawai_M.KdDetailKategoryPegawai " & _
    "WHERE dbo.RiwayatRealisasiUsulan.IdPegawai = '" & mstrIdPegawai & "' "
    Set rsb = Nothing
    ...
    With dgData
        ...
        .Columns(6).Width = 2000 'detail kategory
        .Columns(7).Width = 1500 'gaji
        .Columns(8).Width = 2000 'keterangan
        .Columns(10).Width = 1500 'tgl disetujui
        .Columns(11).Width = 1500 'tgl berlaku
...
Private Sub subClearData()
    ...
    txtIdPegawai.Text = ""
    txtNoRiwayat.Text = ""
    dcDetailKategory.BoundText = ""
    txtGaji.Text = 0
    dtpTglSK.Value = Format(Now, "dd/mmmm/yyyy")
    dtpSetuju.Value = Format(Now, "dd/mmmm/yyyy")
    dtpBerlaku.Value = Format(Now, "dd/mmmm/yyyy")
    txtTTD.Text = ""
    txtNoSK.Text = ""
    txtKeterangan.Text = ""
End Sub
...
Private Sub txtGaji_LostFocus()
    txtGaji.Text = IIf(Val(txtGaji) = 0, 0, Format(txtGaji, "#,###"))
End Sub
...
Sub SetComboDetailKategori()
    On Error GoTo hell
    Set rs = Nothing
    rs.Open "Select KdDetailKategoryPegawai, DetailKategoryPegawai from DetailKategoryPegawai_M where KdKategoryPegawai='2' order by DetailKategoryPegawai ", dbConn, , adLockOptimistic
    Set dcDetailKategory.RowSource = rs
    dcDetailKategory.ListField = rs.Fields(1).Name
    dcDetailKategory.BoundColumn = rs.Fields(0).Name
    Set rs = Nothing
    Exit Sub
hell:
    Call msubPesanError
...
Private Function sp_RiwayatRealisasi() As Boolean
    ...
    With adoComm
        ...
        If IsNull(dtpSetuju.Value) Then
            .Parameters.Append .CreateParameter("TglDiSetujui", adDate, adParamInput, , Null)
        Else
            .Parameters.Append .CreateParameter("TglDiSetujui", adDate, adParamInput, , Format(dtpSetuju.Value, "yyyy/MM/dd"))
        End If
        ...
        If IsNull(dtpBerlaku.Value) Then
            .Parameters.Append .CreateParameter("TglBerlaku", adDate, adParamInput, , Null)
        Else
            .Parameters.Append .CreateParameter("TglBerlaku", adDate, adParamInput, , Format(dtpBerlaku.Value, "yyyy/MM/dd"))
        End If
        .Parameters.Append .CreateParameter("TugasPekerjaan", adVarChar, adParamInput, 150, Null)
        .Parameters.Append .CreateParameter("KdStatusDiSetujui", adChar, adParamInput, 2, Null)
        .Parameters.Append .CreateParameter("KdDKategoryPDiSetujui", adVarChar, adParamInput, 2, dcDetailKategory.BoundText)
        .Parameters.Append .CreateParameter("GajiPokokDiSetujui", adCurrency, adParamInput, , CCur(txtGaji.Text))
        .Parameters.Append .CreateParameter("KdPangkatDiSetujui", adVarChar, adParamInput, 2, Null)
        ...
        .Parameters.Append .CreateParameter("NoRiwayatUsulan", adChar, adParamInput, 10, Trim(txtNoRiwayatUsulan.Text))

        .ActiveConnection = dbConn
        .CommandText = "AU_HRD_RiwayatRealisasi"
        .CommandType = adCmdStoredProc
        .Execute
