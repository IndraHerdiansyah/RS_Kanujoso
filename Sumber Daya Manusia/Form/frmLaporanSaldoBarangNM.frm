...
Private Sub dcGroup_GotFocus()
    ...
    If optPabrik.Value = True Then
        Call msubDcSource(dcGroup, rs, "SELECT KdPabrik, NamaPabrik FROM Pabrik ORDER BY NamaPabrik")
    ElseIf optAsalBarang.Value = True Then
        Call msubDcSource(dcGroup, rs, "SELECT KdAsal, NamaAsal FROM AsalBarang where kdinstalasi='05' ORDER BY NamaAsal")
    ElseIf optJenisBarang.Value = True Then
        Call msubDcSource(dcGroup, rs, "SELECT KdJenisBarang, JenisBarang FROM JenisBarang ORDER BY JenisBarang")
    ElseIf optStatusBarang.Value = True Then
        Call msubDcSource(dcGroup, rs, "SELECT KdStatusBarang, StatusBarang FROM StatusBarang ORDER BY StatusBarang")
    ElseIf optGolonganBarang.Value = True Then
        Call msubDcSource(dcGroup, rs, "SELECT KdGolBarang, GolonganBarang FROM GolonganBarang ORDER BY GolonganBarang")
    Else
        Exit Sub
    End If
