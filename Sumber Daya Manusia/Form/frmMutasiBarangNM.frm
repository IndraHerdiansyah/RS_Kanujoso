...
Private Sub dcverifikator_GotFocus()
       Call msubDcSource(dcverifikator, rs, "SELECT idpegawai, namalengkap FROM datapegawai")
...
Private Sub dcverifikator_KeyPress(KeyAscii As Integer)
    ...
    If KeyAscii = 13 Then
      If dcverifikator.MatchedWithList = True Then txtNamaBarang.SetFocus
       Call msubDcSource(dcverifikator, rs, "SELECT idpegawai, namalengkap FROM datapegawai where namalengkap like '%" & dcverifikator & "%'")
...
On Error GoTo errLoad
    ...
    strSQL = "SELECT  [Nama Barang], Satuan, DetailJenisBarang AS [Jenis Barang], KdBarang, KdAsal,AsalBarang, NoRegisterAsset,jmlStok FROM V_CariBarangNonMedisx" & _
        " WHERE kdruangan='" & mstrKdRuangan & "' AND [Nama Barang] LIKE '%" & txtNamaBarang.Text & "%' AND NoRegisterAsset <> '' " & _
