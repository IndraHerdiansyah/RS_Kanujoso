...
Begin VB.Form frmRiwayatUsulanTaperum 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Medifirst2000 - Riwayat Usulan TAPERUM"
   ClientHeight    =   8460
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   9735
   Icon            =   "frmRiwayatUsulanTaperum.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ...
   Begin VB.Image Image2 
      Height          =   945
      Left            =   7920
      Picture         =   "frmRiwayatUsulanTaperum.frx":0CCA
      Stretch         =   -1  'True
      Top             =   0
      Width           =   1875
   End
   Begin VB.Image Image3 
      Height          =   975
      Left            =   0
      Picture         =   "frmRiwayatUsulanTaperum.frx":1A52
      Stretch         =   -1  'True
      Top             =   0
      Width           =   1800
   End
   Begin VB.Image Image1 
      Height          =   975
      Left            =   1800
      Picture         =   "frmRiwayatUsulanTaperum.frx":4413
      Stretch         =   -1  'True
      Top             =   0
      Width           =   9495
   End
End
Attribute VB_Name = "frmRiwayatUsulanTaperum"
...
Private Sub dgData_RowColChange(LastRow As Variant, ByVal LastCol As Integer)
    ...
    With dgData
        ...
        txtPagu.Text = .Columns(5).Value
        If IsNull(.Columns(6).Value) Then txtPagu.Text = "" Else txtPagu.Text = .Columns(6).Value
...
Private Sub subLoadRiwayatUsulan()
    ...
    strSQL = "SELECT NoRiwayat, IdPegawai, TglSK, NoSK, TandaTanganSK, TotalPaguUsulan, BankPenyalur, KeteranganLainnya, NoRiwayatRealisasi " & _
    "From RiwayatUsulan " & _
    "WHERE (TotalPaguUsulan Is Not Null) " & _
    "AND IdPegawai = '" & mstrIdPegawai & "' "
    Set rsx = Nothing
    Call msubRecFO(rsx, strSQL)
    Set dgData.DataSource = rsx
    With dgData
        ...
        .Columns(5).Width = 1500 'total pagu
        .Columns(6).Width = 1500 'bank penyalur
...
Private Sub txtPagu_KeyPress(KeyAscii As Integer)
    If KeyAscii = 13 Then txtBank.SetFocus
End Sub

Private Sub txtPagu_LostFocus()
    txtPagu.Text = IIf(Val(txtPagu) = 0, 0, Format(txtPagu, "#,###"))
End Sub
...
Private Function sp_RiwayatUsulan() As Boolean
    ...
    With adoComm
        ...
        .Parameters.Append .CreateParameter("GajiPokokUsulan", adCurrency, adParamInput, , CCur(txtPagu.Text))
        ...
        .Parameters.Append .CreateParameter("TotalPaguUsulan", adCurrency, adParamInput, , CCur(txtPagu.Text))
        .Parameters.Append .CreateParameter("BankPenyalur", adVarChar, adParamInput, 50, IIf(txtBank.Text = "", Null, Trim(txtBank.Text)))
