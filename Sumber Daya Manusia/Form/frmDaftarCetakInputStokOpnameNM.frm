...
If KeyAscii = 39 Then KeyAscii = 0
    If KeyAscii = 13 Then
        If Len(Trim(dcJenisAkhir.Text)) = 0 Then cmdCetak.SetFocus: Exit Sub
        If dcJenisAkhir.MatchedWithList = True Then cmdCetak.SetFocus: Exit Sub
        Call msubRecFO(dbRst, "SELECT KdDetailJenisBarang, DetailJenisBarang FROM V_DetailJenisBrgPerKelompokBrg WHERE DetailJenisBarang LIKE '%" & dcJenisAkhir.Text & "%' ")
        If dbRst.EOF = True Then Exit Sub
        dcJenisAkhir.BoundText = dbRst(0).Value
        dcJenisAkhir.Text = dbRst(1).Value
    End If
    Exit Sub
errLoad:
    Call msubPesanError
...
If KeyAscii = 39 Then KeyAscii = 0
    If KeyAscii = 13 Then
        If Len(Trim(dcJenisAwal.Text)) = 0 Then dcJenisAkhir.SetFocus: Exit Sub
        If dcJenisAwal.MatchedWithList = True Then dcJenisAkhir.SetFocus: Exit Sub
        Call msubRecFO(dbRst, "SELECT KdDetailJenisBarang, DetailJenisBarang FROM V_DetailJenisBrgPerKelompokBrg WHERE DetailJenisBarang LIKE '%" & dcJenisAwal.Text & "%' ")
        If dbRst.EOF = True Then Exit Sub
        dcJenisAwal.BoundText = dbRst(0).Value
        dcJenisAwal.Text = dbRst(1).Value
    End If
    Exit Sub
errLoad:
    Call msubPesanError
