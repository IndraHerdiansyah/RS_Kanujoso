...
Begin VB.Form frmRiwayatTempatBertugas 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Medifirst2000 - Riwayat Tempat Bertugas/Mutasi"
   ClientHeight    =   7380
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   10335
   Icon            =   "frmRiwayatTempatBertugas.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ...
   Begin VB.Image Image2 
      Height          =   945
      Left            =   8520
      Picture         =   "frmRiwayatTempatBertugas.frx":0CCA
      Stretch         =   -1  'True
      Top             =   0
      Width           =   1875
   End
   Begin VB.Image Image3 
      Height          =   975
      Left            =   0
      Picture         =   "frmRiwayatTempatBertugas.frx":1A52
      Stretch         =   -1  'True
      Top             =   0
      Width           =   1800
   End
   Begin VB.Image Image1 
      Height          =   975
      Left            =   1800
      Picture         =   "frmRiwayatTempatBertugas.frx":4413
      Stretch         =   -1  'True
      Top             =   0
      Width           =   9495
   End
End
Attribute VB_Name = "frmRiwayatTempatBertugas"
...
'    If dcKdRuangan.Text <> "" Then
'        If Periksa("datacombo", dcKdRuangan, "Nama Ruangan Tidak Terdaftar") = False Then Exit Sub
'    End If
'    If dcKdJabatan.Text <> "" Then
'        If Periksa("datacombo", dcKdJabatan, "Jabatan Tidak Terdaftar") = False Then Exit Sub
'    End If
'
'    If Periksa("datacombo", dcKdRuangan, "Silahkan isi nama ruangan ") = False Then Exit Sub
'    If Periksa("datacombo", dcKdJabatan, "Silahkan isi nama jabatan ") = False Then Exit Sub
...
Private Sub dcKdJabatan_KeyPress(KeyAscii As Integer)
On Error GoTo hell
    If KeyAscii = 39 Then KeyAscii = 0
    If KeyAscii = 13 Then
        If Len(Trim(dcKdJabatan.Text)) = 0 Then dcEselon.SetFocus: Exit Sub
        If dcKdJabatan.MatchedWithList = True Then dcEselon.SetFocus: Exit Sub
        strSQL = "Select KdJabatan,NamaJabatan from Jabatan WHERE (NamaJabatan LIKE '%" & dcKdJabatan.Text & "%')"
        Call msubRecFO(rs, strSQL)
        If rs.EOF = True Then Exit Sub
        dcKdJabatan.BoundText = rs(0).Value
        dcKdJabatan.Text = rs(1).Value
    End If
    Exit Sub
hell:
    Call msubPesanError
...
Private Sub dcKdRuangan_KeyPress(KeyAscii As Integer)
On Error GoTo hell
    If KeyAscii = 39 Then KeyAscii = 0
    If KeyAscii = 13 Then
        If Len(Trim(dcKdRuangan.Text)) = 0 Then txtSatuanKerja.SetFocus: Exit Sub
        If dcKdRuangan.MatchedWithList = True Then txtSatuanKerja.SetFocus: Exit Sub
        strSQL = "Select KdRuangan,NamaRuangan from Ruangan WHERE (NamaRuangan LIKE '%" & dcKdRuangan.Text & "%')"
        Call msubRecFO(rs, strSQL)
        If rs.EOF = True Then Exit Sub
        dcKdRuangan.BoundText = rs(0).Value
        dcKdRuangan.Text = rs(1).Value
    End If
    Exit Sub
hell:
    Call msubPesanError
...
Private Sub dgTempatBertugas_RowColChange(LastRow As Variant, ByVal LastCol As Integer)
    ...
    With dgTempatBertugas
        ...
        If IsNull(.Columns(4).Value) Then txtSatuanKerja.Text = "" Else txtSatuanKerja.Text = .Columns(4).Value
        dcKdJabatan.BoundText = .Columns(15).Value
        If IsNull(.Columns(16).Value) Then dcEselon.BoundText = "" Else dcEselon.BoundText = .Columns(16).Value
        If IsNull(.Columns(7).Value) Then dtpTglSK.Value = Null Else dtpTglSK.Value = .Columns(7).Value
        If IsNull(.Columns(8).Value) Then txtNoSuratKeputusan.Text = "" Else txtNoSuratKeputusan.Text = .Columns(8).Value
        If IsNull(.Columns(9).Value) Then txtTTD.Text = "" Else txtTTD.Text = .Columns(9).Value
        dtpTglMulai.Value = dgTempatBertugas.Columns(10).Value
        If IsNull(.Columns(11).Value) Then dtpTglAkhir.Value = Null Else dtpTglAkhir.Value = .Columns(11).Value
        If IsNull(.Columns(12).Value) Then txtKeterangan.Text = "" Else txtKeterangan.Text = .Columns(12).Value
...
Sub SetComboRuangan()
    On Error GoTo hell
    Set rs = Nothing
    rs.Open "Select * from Ruangan where StatusEnabled='1' order by NamaRuangan ASC", dbConn, , adLockOptimistic
...
Sub SetComboJabatan()
    On Error GoTo hell
    Set rs = Nothing
    rs.Open "Select * from Jabatan where StatusEnabled='1' order by NamaJabatan ASC", dbConn, , adLockOptimistic
...
Sub SetComboEselon()
    On Error GoTo hell
    Set rs = Nothing
    rs.Open "Select * from Eselon where StatusEnabled='1' order by NamaEselon ASC", dbConn, , adLockOptimistic
...
Private Function sp_RiwayatTempatBertugas() As Boolean
    ...
    With adoComm
        ...
        .Parameters.Append .CreateParameter("KdRuangan", adChar, adParamInput, 3, IIf(dcKdRuangan.Text = "", Null, dcKdRuangan.BoundText))
        ...
        .Parameters.Append .CreateParameter("KdJabatan", adVarChar, adParamInput, 5, IIf(dcKdJabatan.Text = "", Null, dcKdJabatan.BoundText))
