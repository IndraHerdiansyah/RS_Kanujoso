...
Private Sub txtNamaBarang_Change()
    ...
    If dcStatusBarang.BoundText = "02" Then
        ...
        strSQL = "select  distinct DetailJenisbarang as [Jenis Barang], NamaBarang as [Nama Barang], Kekuatan, SatuanJmlB as Satuan, (SUM(JmlStokFIFO) + (JmlStokRuangan)) AS JmlStok, KdBarang, KdSatuanJmlB, KdRuangan " & _
        " from V_InfoStokGudangFarmasiFIFO " & _
        " where NamaBarang like '%" & txtNamaBarang.Text & "%' AND KdRuangan = '" & dcRuanganTujuan.BoundText & "' " & _
        " GROUP BY DetailJenisbarang, NamaBarang, Kekuatan, SatuanJmlB, KdBarang, KdSatuanJmlB, KdRuangan,JmlStokRuangan" & _
        " ORDER BY NamaBarang"
        ...
        With dgObatAlkes
            ...
            .Columns("JmlStok").NumberFormat = "#,##0.00"
            .Columns("JmlStok").Alignment = dbgRight
        End With
    ElseIf dcStatusBarang.BoundText = "01" Then

        strSQL = "select  distinct DetailJenisbarang as [Jenis Barang], NamaBarang as [Nama Barang], Kekuatan, SatuanJmlB as Satuan, (SUM(JmlStokFIFO) + (JmlStokRuangan)) AS JmlStok, KdBarang, KdSatuanJmlB, KdRuangan " & _
        " from V_InfoStokGudangUmumFIFO " & _
        " where NamaBarang like '%" & txtNamaBarang.Text & "%' AND KdRuangan = '" & dcRuanganTujuan.BoundText & "' " & _
        " GROUP BY DetailJenisbarang, NamaBarang, Kekuatan, SatuanJmlB, KdBarang, KdSatuanJmlB, KdRuangan,JmlStokRuangan" & _
        " ORDER BY NamaBarang"
        Call msubRecFO(dbRst, strSQL)
...
Private Function sp_DetailOrderRuangan(f_KdBarang As String, f_JumlahBarang As Double, f_Status As String) As Boolean
    On Error GoTo errLoad
