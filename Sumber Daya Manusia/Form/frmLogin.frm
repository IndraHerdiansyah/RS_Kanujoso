...
Private Sub cmdOK_Click()
    ...
    Do While rsLogin.EOF = False
        If UCase(strUser) = UCase(rsLogin!username) And strPass = Crypt(rsLogin!Password) Then
            'Username sedang aktif/digunakan di komputer lain
            '            If rsLogin!Status = 1 Then
            '                MsgBox "Username sedang aktif", vbCritical, "Username Error"
            '                Exit Sub
            '            End If
            '            strStatus = "1"
...
UserPermited:
            ...
            '            'restrict changes on old patients
...
On Error Resume Next
    If KeyAscii = 13 Then
        ...
        strSQL = "select KdRuangan, NamaRuangan from V_LoginAplikasiHRD WHERE (NamaRuangan LIKE '%" & dcRuangan.Text & "%')"
...
Private Sub Form_Load()
    ...
    rs.Open "select * from V_LoginAplikasiHRD ORDER BY NamaRuangan ASC", dbConn, adOpenStatic, adLockReadOnly
    ...
    strSQL = "select * from SettingGlobal where Prefix='RealDB'"
    Call msubRecFO(rs, strSQL)
    If UCase(rs!Value) <> UCase(strDatabaseName) Then
