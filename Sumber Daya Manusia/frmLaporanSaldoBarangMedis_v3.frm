...
Dim sFltrSaldoBy As String
    ...
    If chkGroupBy.Value = Checked Then
        ...
        End If
    'Tambh by JDR agar Judul sesuai dengan Group by
        fgData.TextMatrix(0, 10) = sFltrSaldoBy
    Else
        sFltrGroupBy = ""
        sFltrSaldoBy = ""
         'fgData.ColWidth(0, 10) = 0
    End If
    ...
    Else
        'intTgl = intHr
        'Jika pake intHr maka tanggal akan dikurangi 1 hari (2009/12/14)
        intTgl = CInt(Day(dtTgl))
        intBln = CInt(Month(dtTgl))
        intThn = CInt(Year(dtTgl))
    End If
    'Kondisi bay JDR karen variable Int tgllast haya dipakai jika inthari =0
    If intHr = 0 Then
        intTgl = intTglLast
    End If
    ...
    Set rs = Nothing
    strSQL = "Select KdBarang,KdAsal From V_StokBarangMedisRuanganLengkap Where KdRuangan='" & mstrKdRuangan & "'" & sFltrAsalBrg & sFltrGroupBy
    Call msubRecFO(rs, strSQL)
    If rs.EOF = True Then
...
'            .TextMatrix(i, 2) = rs("KdAsal")
            ...
            ElseIf optBulan.Value = True Then
                strSQL = "SELECT KdBarang, KdAsal,NamaBarang,NamaAsal, CASE (SELECT SIGN(SUM(JmlStok)) FROM LaporanSaldoBarangMedisApotikNRuangan_V WHERE TglKirim BETWEEN '" & Format(intThn & "/" & intBln & "/01", "yyyy/MM/dd 00:00:00") & "' AND '" & Format(intThn & "/" & intBln & "/" & intTgl, "yyyy/MM/dd 23:59:59") & "' AND KdBarang = '" & rs("KdBarang") & "' AND KdAsal='" & rs("KdAsal") & "') " & _
            ...
            For j = 1 To dbRst.RecordCount
                .TextMatrix(k, 1) = dbRst("KdBarang")
                .TextMatrix(k, 2) = dbRst("KdAsal")
                .TextMatrix(k, 3) = dbRst("NamaBarang")
                .TextMatrix(k, 4) = dbRst("NamaAsal")
                .TextMatrix(k, 5) = IIf(IsNull(dbRst("SaldoAwal")), "0", dbRst("SaldoAwal"))
                .TextMatrix(k, 6) = dbRst("JmlTerima")
                .TextMatrix(k, 7) = dbRst("JmlKeluar")
                .TextMatrix(k, 8) = IIf(IsNull(dbRst("SaldoAwal")), "0", dbRst("SaldoAwal")) + dbRst("JmlTerima") - dbRst("JmlKeluar")
                .TextMatrix(k, 9) = dbRst("HargaJual")
                'dbRst ("DiscountGive")
                'tambah by JDR jika Jenis barang tidak di pilih maka tampil semua
                If sFltrSaldoBy = "" Then
                    .TextMatrix(k, 10) = IIf(IsNull(dbRst("JenisBarang")), "", dbRst("JenisBarang"))
                Else
                    .TextMatrix(k, 10) = IIf(IsNull(dbRst("" & sFltrSaldoBy & "")), "", dbRst("" & sFltrSaldoBy & ""))
                End If
                
                .TextMatrix(k, 11) = dbRst("KdRuanganTujuan")
...
Sub setgrid()
    With fgData
        ...
        .Rows = 2
        .Cols = 12
        'Tambah KdRuangan by JDR (2009-12-15)
