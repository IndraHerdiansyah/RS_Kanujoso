...
Private Sub cmdSimpan_Click()
    ...
    If Periksa("Datacombo", dcRuanganTujuan, "Ruangan Salah") = False Then Exit Sub
    If dcRuanganTujuan.Text = "" Then MsgBox " Ruangan tujuan pesanan harus diisi", vbExclamation, "Validasi": Exit Sub
    
    If fgData.TextMatrix(1, 0) = "" Then MsgBox "Data barang harus diisi", vbExclamation, "Validasi": Exit Sub
    
    For i = 1 To fgData.Rows - 2
        With fgData
            If fgData.TextMatrix(i, 2) = "" Then MsgBox "Jumlah barang tidak boleh kosong / 0", vbExclamation, "Validasi": Exit Sub
        End With
    Next i
    ...
    End If
    Call cmdBatal_Click
    dcStatusBarang.Enabled = True
    bolCekStokPesanBarang = False
    Exit Sub
errLoad:
    Call msubPesanError
End Sub
...
If dcStatusBarang.BoundText = "02" Then
    ...
    If Val(txtStock.Text) < Val(txtJumlah) Then
        MsgBox "Jumlah qty melebihi jumlah stok", vbInformation
        Exit Sub
    End If
...
Private Sub dcRuanganTujuan_Change()
    If bolCekStokPesanBarang = True Then
    Dim i As Integer
    For i = 1 To fgData.Rows - 2
       If dcStatusBarang.BoundText = "02" Then
            If (bolStatusFIFO = True) Then
                strSQL = "select JmlStokOnHand from stokRuanganFIFO where kdBarang='" & fgData.TextMatrix(i, 4) & "' and kdRUangan='" & dcRuanganTujuan.BoundText & "' and kdasal='" & fgData.TextMatrix(i, 6) & "' "
            ElseIf (bolStatusFIFO = False) Then
                strSQL = "select JmlStokOnHand from stokRuangan where kdBarang='" & fgData.TextMatrix(i, 4) & "' and kdRUangan='" & dcRuanganTujuan.BoundText & "' and kdasal='" & fgData.TextMatrix(i, 6) & "'"
            End If
            
            Call msubRecFO(rsH, strSQL)
            If rsH.EOF = True Then
                 fgData.TextMatrix(i, 1) = "0"
            Else
                fgData.TextMatrix(i, 1) = rsH.Fields("JmlStokOnHand").Value
            
            End If
       Else
            strsqlx = "select  SUM(jmlstok) AS jmlstok " & _
                      " from V_CariBarangNonMedis " & _
                      " where KdBarang='" & fgData.TextMatrix(i, 4) & "' And KdRuangan='" & dcRuanganTujuan.BoundText & "' "
            
            Call msubRecFO(rsH, strsqlx)
            If rsH.EOF = True Then
                 fgData.TextMatrix(i, 1) = "0"
            Else
                fgData.TextMatrix(i, 1) = IIf(IsNull(rsH.Fields("jmlstok").Value), "0", rsH.Fields("jmlstok").Value)
...
Private Sub Form_Unload(Cancel As Integer)
  'If txtNamaFormPengirim.Text = "frmMonitoringStokBarang" Then frmMonitoringStokBarang.Enabled = True
  If txtNamaFormPengirim.Text = "frmMonitoringStokBarangNonMedis" Then frmMonitoringStokBarangNonMedis.Enabled = True
End Sub
        ...
        Select Case fgData.Col
            Case 2
                If txtIsi.Text = "" Then
                  txtIsi.Text = 0
                End If
                If Val(txtIsi.Text) > Val(fgData.TextMatrix(fgData.row, 1)) Then
                MsgBox "Jumlah Melebihi stok Ruangan Tujuan", vbInformation
                Exit Sub
                
                End If
                If txtIsi.Text = 0 Or txtIsi = "" Then MsgBox "Jumlah barang tidak boleh 0 atau kosong ?", vbCritical, "Validasi": Exit Sub
                If Val(txtIsi.Text) = 0 Then txtIsi.Text = 0
                fgData.TextMatrix(fgData.row, fgData.Col) = msubKonversiTitikKoma(txtIsi.Text)
                'fgData.Col = 2
                'fgData.SetFocus
                'fgData.Col = 3
...
Private Sub txtNamaBarang_Change()
    ...
    ElseIf dcStatusBarang.BoundText = "01" Then
        ...
        strSQL = "SELECT distinct DetailJenisBarang, NamaBarang, SatuanJmlB, SUM(JmlStok) AS JmlStok, KdBarang," & _
        ...
        " GROUP BY DetailJenisBarang, NamaBarang, SatuanJmlB, KdBarang, KdSatuanJmlB, KdRuangan" & _
        " ORDER BY NamaBarang "
...
Private Sub subLoadDcSource()
    ...
    Call msubDcSource(dcStatusBarang, rs, "SELECT KdKelompokBarang, KelompokBarang FROM KelompokBarang Where KdKelompokBarang in ('01','03') ORDER BY KelompokBarang")
...
Private Function sp_StrukOrder() As Boolean
    ...
    With dbcmd
        ...
        .Parameters.Append .CreateParameter("TglJatuhTempo", adDate, adParamInput, , Null)
